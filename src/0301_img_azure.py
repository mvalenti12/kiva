...
logging.basicConfig(filename=f'tmp/log_msft_{experiment_id}_{time.strftime("%Y%m%d-%H%M%S")}.txt',
                    level=logging.INFO, 
...
if len(res)>0:
  df = pd.DataFrame(res)
  filename = OUTPUT_DIR  + experiment_id + '_' + time.strftime("%Y%m%d-%H%M%S") + '_msft.csv'
  df.to_csv(filename)
